...
inline char* decompress(const char* source, unsigned long source_len, unsigned long* dest_len) {
    ...
    *dest_len = buffer_size;
    ...
    while (state != Z_OK) {
        switch (state) {
        case Z_MEM_ERROR:
            throw std::exception("No enough memory for decompression.");
        case Z_DATA_ERROR:
            throw std::exception("Compressed data is broken.");
        case Z_BUF_ERROR:
            ...
            *dest_len = buffer_size;
            ...
            break;
        default:
            throw std::exception("Impossible path in decompressing.");
        }
    }
